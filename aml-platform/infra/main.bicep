...
//
//     Azure Cloud Shell creates its own `cloud-shell-storage-<region>`
...
//
// DuckDB's `credential_chain` resolves managed identity through IMDS and has
...
// WHAT IS DEPLOYED IS NOT THIS TEMPLATE. The live group was created by
// `az vm create`, so it holds aml-vmNSG / aml-vmVMNic / aml-vmVNET /
// aml-vmPublicIP -- the CLI defaults -- and NOT this file's named
// resources or its NAT gateway.
//
// A STATIC PUBLIC IP IS ATTACHED to the running VM. Inbound 22 is Deny
// from * and everything else is DenyAllInBound, so it is not reachable --
// but it exists and it bills. Do not read this template's intent as the
// state: re-deploying it into that group would try to move the VM onto a
// new NIC.
  ...
  properties: {
    ...
    storageProfile: {
      imageReference: {
        ...
        sku: '22_04-lts-gen2'
        // THE ONE UNPINNED ARTEFACT IN A DIGEST-PINNING PROJECT.
        // Every GitHub Action here is pinned by SHA and the container
        // base is pinned by digest; this resolves at deploy time. It
        // last resolved to 22.04.202608060. Pinning it means editing
        // this line on every base-image refresh, which is why it is
        // still 'latest' -- recorded rather than hidden.
        version: 'latest'
      }
      osDisk: {
