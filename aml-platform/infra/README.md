...
> The VM and its disks account for almost all of the cost. The registry is
> **off by default** (`deployRegistry=false`): `az acr build` returns
> `TasksOperationsNotAllowed` on a trial subscription, so provisioning it
> buys nothing. See
...
| blob storage, ADLS Gen2 (6.59 GB at rest) | 0.00019 |
...
> Azure Cloud Shell creates its own `cloud-shell-storage-<region>` resource
...
> default (`deployRegistry=false`), so `registryLoginServer` is empty with
> the documented deployment, and on a trial subscription ACR Tasks return
