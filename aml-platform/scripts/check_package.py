...
reader who never sees the repository, so a README that presents the
REPOSITORY layout as the package contents reads as a claim to redistribute
data whose licence status this project records as UNREVIEWED. Neither
distribution holds any of it: the wheel holds `aml/` and its dist-info, and
the sdist adds this README and `pyproject.toml`.
...
# Absent from BOTH. `results_archive` is the load-bearing one; the rest are
# repository directories that a README can easily be read as promising.
