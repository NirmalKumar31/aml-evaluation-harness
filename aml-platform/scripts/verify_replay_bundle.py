...
WHAT IS AND IS NOT AN INPUT. The recomputation reads `rank` and `y`, never
`score` -- the bundle's ranking is already materialised, and re-deriving it
from scores would test pandas rather than the archive. So permuting the
`score` column changes nothing here, and an audit correctly observed that a
score shuffle passes 42/42. That is by construction, not a gap: permute the
`rank` column instead, or recompute rank from shuffled scores, and the check
fails loudly (17 of 42 on the demo bundle).

WHAT THE DEMO BUNDLE CAN AND CANNOT SHOW. On the synthetic corpus the budget
never binds -- 6 test days, 14 positive account-days, at most 240 account-days
in a day, against budgets up to 1000 -- so most metrics saturate and carry
little discriminating power. `make replay-demo` demonstrates that the
MECHANISM composes end to end with no dataset. It is not evidence that the
metric definitions are right; the archived bundles at real budgets are.
"""
