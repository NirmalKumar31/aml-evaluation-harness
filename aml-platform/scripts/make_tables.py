...
USAGE
    ...
    non-zero if any number in them disagrees with the artifacts, so a stale
    published figure fails CI rather than waiting to be noticed.
"""
...
def _divides_by_power_of_ten(expr: str) -> float | None:
    ...
    Spelling-independent on purpose. A regex over the literal text is walked
    through by `/ 100.0`, `/(50*2)` and `/1e2`, all of which are the same
    divisor as the `/100` it catches.
    """
    import ast
...
def arithmetic_objection(item: str, known: dict) -> str | None:
    ...
    # A POWER OF TEN IS A UNIT CONVERSION, NOT A DERIVATION. Matching the
    # divisor's spelling is evaded three ways -- `/ 100.0`, `/(50*2)` and
    # `/1e2` -- so the divisor is evaluated instead.
...
MONEY = re.compile(r"\$\s?([0-9]+\.[0-9]{2,5})\b")
# `117.56 resource-hours`, `121.96 h`, `99.99 hours`.
HOURS = re.compile(r"\b([0-9]+\.[0-9]{1,2})\s*(?:resource-)?h(?:ours?|rs?)?\b")
...
LIFT = re.compile("(?<![0-9.,])([0-9]+\\.[0-9]{1,6})\\s*(?:\u00d7|x)(?![\\w.])")
...
DEC = re.compile(r"(?<![0-9.,])([1-9][0-9]*\.[0-9]{2,17})(?![0-9]|\.[0-9])")
# 3-17 decimals, not 3-6. At 3-6 every value with seven or more decimals was
# invisible -- 16 live tokens, and they were the REPRODUCIBILITY EVIDENCE:
# `docs/RESULT_LINEAGE.md` prints the lineage comparison at 12 decimals four
# lines under "reproduces the older lineage to the last digit", and
# `docs/ENGINEERING_NOTES.md` prints the arm64-vs-amd64 identity table at 15.
# The project's two strongest reproducibility claims rested on values its own
# gate could not see. `_remember` below indexes the same range, without which
# widening here only manufactures false "in no artifact" findings.
BARE = re.compile(r"\b0\.[0-9]{3,17}\b")
...
LIFT_INT = re.compile("(?<![0-9.,])([0-9][0-9,]*)\\s*(?:\u00d7|x)(?![\\w.])")
...
_NOISE = (
    ...
    # `v` must be glued to its digits. As a free alternative with `\s*` it ate
    # the word "versus": `0.57060 v 0.49147` yielded only the first token,
    # deleting the comparator from this repository's single most common
    # sentence shape.
    re.compile(r"\b(?:Python|python|version)\s*\d+\.\d+(?:\.\d+)*|\bv\d+\.\d+(?:\.\d+)*"),
    re.compile(r"\b\d+\.\d+\.\d+\b"),               # semver, §1.2.3
    # THIS ONCE READ `` `[^`]*@\d[^`]*` `` AND THAT DELETED THE PROJECT'S
    # OWN NOTATION. It exists to ignore pinned tool versions like
    # `markdownlint-cli2@0.14.0`. But EVERY metric here is named `name@k`, so
    # any value written inside backticks beside its metric -- which is the
    # house style -- was stripped before matching. Proven by injection: five
    # fabricated figures written as `` `precision@50 = 0.99999` `` produced
    # **0 findings**; the same figures unbackticked produced findings
    # immediately. The gate the whole project rests on could be bypassed with
    # two backticks.
    #
    # First fix: require `@<digits>.<digits>`, so a budget like `@50` no
    # longer matched. THAT WAS STILL WRONG, twice over. It kept deleting
    # every value in any backticked span holding a two-part `@X.Y` --
    # `` `fpr@0.01 = 0.99903` `` and `` `precision@50.0 = 0.99904` `` both
    # went to zero tokens -- and it was REDUNDANT, because `\b\d+\.\d+\.\d+\b`
    # above already strips three-part pins. Measured: no gated document
    # contains a backticked two-part pin at all (the only pin in the repo,
    # `markdownlint-cli2@0.14.0`, lives in `.github/workflows/gates.yml`,
    # which is not a gated document), and deleting the rule leaves the gate's
    # output identical while closing both holes.
    #
    # So the rule is gone rather than narrowed. Pins remain ignored:
    # `lightgbm@4.5.0` via the three-part version pattern, `lightgbm@4.5`
    # because one-decimal values are outside the extractor's domain anyway.
    # (the `@`-span rule that used to live here is DELETED -- see above)                    # pinned `tool@0.14.0`
)
...
# Unchecked, the same marker on a line stating the withdrawn claim as the
# CURRENT reading exempts the claim instead of labelling it. A line claiming
# to be history has to read like history.
RETRACTION_CUES = (
    # "historical" IS DELIBERATELY NOT HERE ANY MORE. The marker string
    # `<!-- historical -->` contains it, so including it made
    # `HISTORICAL in line and any(cue in line)` equivalent to
    # `HISTORICAL in line` -- a tautology that turned the marker into an
    # unconditional bypass for the whole retraction registry and made the
    # "does not narrate" diagnostic unreachable. The marker is stripped
    # before this list is applied; these must match the PROSE.
    "used to", "earlier version", "previously", "until the", "until 20",
    "was withdrawn", "is withdrawn", "withdrawn", "withdraws",
    "retracted", "retraction", "no longer", "this line said",
    "this bullet said", "this section said", "this section reported",
    "this section was", "this read", "this said", "this paragraph",
    "once read", "once said", "was headed", "had been", "superseded",
        "was wrong", "is gone", "former",
    # An arrow is how this prose writes "the old value became the new one",
    # and a fenced block of superseded numbers is headed further up than one
    # line, which is why the window above is nine lines rather than four.
    "withdraw",
    # More of the same, taken from how these documents actually narrate a
    # superseded value: a rerun that changed a verdict, a corrected
    # figure, a numbered attempt that did not survive.
    "rerun", "verdicts changed",
)
# ---------------------------------------------------------------------------
...
# ENTITIES ARE UNESCAPED, NOT DELETED. Deleting them turned a
# render-different attack into a DIFFERENT-NUMBER read, which is the worse
# half of the class this block exists to close: `0.4404&#53;` renders 0.44045
# and the gate read `0.4404` -- an artifact-backed prefix -- so the line
# passed AND counted itself as checked.
# `_EMPHASIS_IN_NUMBER` takes any run of markers, not one or two: `***` and
# `___` both slipped through `{1,2}`.
_EMPHASIS_IN_NUMBER = re.compile(r"(\d)[*_]+(?=\d)|(?<=\d)[*_]+(\d)")
# ...and a run that TRAILS a number, because `_` is a word character: after
# `0.4404___5___` collapsed to `0.44045___`, `\b` could not close and the
# value went invisible instead of visible-and-wrong.
_EMPHASIS_TRAILING = re.compile(r"(?<=\d)[*_]+(?![*_]*\d)")
# U+FF10-U+FF19 and the fullwidth full stop, mapped to ASCII. Targeted, so
# that superscripts and other compatibility forms are left alone.
_FULLWIDTH = {0xFF10 + i: ord("0") + i for i in range(10)}   # FULLWIDTH DIGIT ZERO..NINE
_FULLWIDTH[0xFF0E] = ord(".")                                # FULLWIDTH FULL STOP

# ONE MULTILINE-SAFE HELPER, because three separate `<!--.*?-->` patterns lived
# in this file and CodeQL flagged all three as `py/bad-tag-filter`: without
# `re.DOTALL` a `.` stops at a newline, so a comment spanning lines is not
# removed.
#
# It was never a security finding. That rule is about sanitising UNTRUSTED HTML
# for rendering; these patterns strip this repository's own `<!-- derived -->`
# and `<!-- source: -->` markers out of this repository's own Markdown, and the
# result is a console report that is never rendered as HTML. The callers also
# pass a single LINE, so a newline inside the match was unreachable.
#
# It is still fixed rather than dismissed. "Unreachable today" is precisely how
# every marker-scoping defect in this file's history began -- a blanket
# `<!-- derived -->` that exempted a whole line, a lookahead that skipped six
# entries in silence, a `findall` that read one marker per line. A caller that
# one day passes a whole document would have inherited the bug, and a helper
# means the next caller inherits the fix instead.
_HTML_COMMENT = re.compile(r"<!--.*?-->", re.DOTALL)


def strip_html_comments(text: str) -> str:
    """Replace every HTML comment with a space, newlines inside it included."""
    return _HTML_COMMENT.sub(" ", text)


# An inline HTML comment splits a value while rendering it whole, and so does
# markdown link syntax wrapped around the first half of one. Same DOTALL
# reasoning as above; the lookahead keeps the three real markers intact.
_HTML_COMMENT_INLINE = re.compile(
    r"<!--(?!\s*(?:derived|source|historical)\b).*?-->", re.DOTALL)
_MD_LINK = re.compile(r"\[([^\]]*)\]\([^)]*\)")

# STOP ENUMERATING NOTATIONS. Three consecutive rounds each found more ways
# to render one number while the gate read another -- HTML tags, entities,
# zero-width characters, emphasis runs, directional isolates, NFKC
# superscripts, combining grapheme joiners, variation selectors, inline
# comments, link syntax. Every fix was a blacklist entry, and the next round
# found the next character.
#
# The closed rule: a DIGIT RUN MAY ONLY TOUCH CHARACTERS THAT ARE VISIBLE AND
# ASCII. Anything else adjacent to a digit -- any invisible, any non-ASCII
# digit, any combining mark -- means the rendered text and the extracted text
# may differ, and the line is REPORTED rather than silently read. That is one
# rule instead of a growing list, and it fails closed.
_SUSPECT_NEAR_DIGIT = re.compile(
    r"[0-9]\s*[^\x00-\x7f]|[^\x00-\x7f]\s*[0-9]")


def obfuscation_objection(raw: str) -> str | None:
    """Is a digit adjacent to something a reader cannot see, or cannot type?"""
    for ch in raw:
        # A non-ASCII DIGIT is reported wherever it appears, not only beside
        # an ASCII one: a value written entirely in Arabic-Indic numerals is
        # invisible to every extractor here, so silence would mean the line
        # renders a number this gate never saw.
        if ch.isdigit() and not ch.isascii():
            return (f"a non-ASCII digit (U+{ord(ch):04X}) appears on this "
                    f"line; the extractors read ASCII digits only, so a value "
                    f"written in them would render without being checked")
        suspect = unicodedata.category(ch) in {"Cf", "Mn", "Me"}
        if suspect and _SUSPECT_NEAR_DIGIT.search(raw):
                return (f"a non-ASCII character (U+{ord(ch):04X}, "
                        f"{unicodedata.category(ch)}) sits next to a digit, so "
                        f"what renders and what this gate reads may differ")
    return None
...
def document_tokens(line: str) -> list[tuple[str, set[str]]]:
    ...
    are the same claim.
    """
    # MARKUP MADE THE GATE READ A DIFFERENT NUMBER FROM THE ONE THAT
    # RENDERS, and pass. `0.44045<sup>999</sup>` renders as 0.44045999; the
    # extractor saw `0.44045`, which IS in an artifact, so the line was
    # reported as CHECKED. The comment on `DEC` already calls this class "not
    # a miss but a DIFFERENT NUMBER, which is worse" -- and five separate
    # notations reached it: HTML tags, HTML entities, zero-width characters,
    # markdown emphasis between digits, and non-ASCII digits.
    #
    # Normalise first, then extract, so what the gate reads is what a reader
    # sees. FULLWIDTH digits are folded; other non-ASCII digit systems
    # (Arabic-Indic, Devanagari) are NOT -- a value written in them stays
    # invisible to the extractor, which is a documented limit rather than a
    # closed hole.
    #
    # Stated precisely on purpose: "refused outright" and "folded by NFKC"
    # are both wrong about the non-fullwidth forms, and an assertion about
    # behaviour the implementation does not have is the defect this block
    # exists to close.
    line = html.unescape(line)
    line = _HTML_TAG.sub("", line)
    # EVERY format character, by Unicode category -- the hand-written class
    # omitted the directional isolates (U+2066-2069) and the embedding and
    # override controls (U+202A-202E), each of which truncated a value while
    # rendering it whole.
    line = "".join(c for c in line if unicodedata.category(c) != "Cf")
    # FULLWIDTH FORMS ONLY, not blanket NFKC. Normalising the whole line
    # folded SUPERSCRIPTS into digits and merged them into the number beside
    # them: `0.7133<SUP FIVE>` became `0.71335`, so a footnote marker after a
    # value silently joined it. That is this block's own defect inverted --
    # instead of reading a prefix of what renders, it read a value that does
    # not render at all.
    line = line.translate(_FULLWIDTH)
    line = _HTML_COMMENT_INLINE.sub("", line)
    line = _MD_LINK.sub(r"\1", line)
    line = _EMPHASIS_IN_NUMBER.sub(r"\1\2", line)
    line = _EMPHASIS_TRAILING.sub("", line)
    for rx in _NOISE:
        line = rx.sub(" ", line)
...
def collect(archive: Path) -> list[dict]:
    ...
    for f in sorted(archive.glob("gold/*/manifest.json")):
        ...
        # ACROSS TWO LINEAGES, and every published 3-seed figure stops being
        # reproducible. It is the competing-lineage problem this tool exists
        # to police, arising inside the tool.
        #
...
PUBLICATION_CORE = (
    "README.md",
    # THE MOST-HARVESTED DOCUMENT. GitHub's "Cite this repository" widget,
    # Zenodo and every citation index read CITATION.cff, so its abstract is
    # the most widely copied prose here and every figure in it is gated.
    "CITATION.cff",
    "CHANGELOG.md",
    "DATA_LICENSE.md",
    ...
    # THE INDEX THAT ROUTES READERS TO THE RESULTS. An index restates the
    # claims of the documents it links to, in words rather than numbers, so
    # the retraction registry -- which matches withdrawn VALUES -- cannot see
    # a withdrawn claim here. It is gated for the values it does carry.
...
def repo_root() -> Path:
    ...
    documents the container does not ship -- CHANGELOG.md and `paper/` are
    not COPYed into it -- so resolving it against /app names more than it can
    read, and a gate that quietly covers less than it says is worse than one
    that fails. `--gate` runs from a checkout; outside one it resolves to
    paths that do not exist and the missing-document check stops it.
    """
...
def publication_docs(root: Path | None = None) -> list[Path]:
    ...
    PRESENT ONLY, because the same inventory is resolved against the container
    image, which ships a subset. Absence is not silence: the caller reports
    how many documents it read, so a file that vanishes by accident shows up
    as a smaller denominator rather than as nothing at all.
    """
    root = repo_root() if root is None else root
    return ([root / p for p in PUBLICATION_CORE if (root / p).is_file()]
...
def marker_report(rows: list[dict], docs: list[Path]) -> int:
    ...
    The way to see at a glance which published values are not artifact-backed,
    and which markers are load-bearing. Run it instead of guessing: a marker
    is easy to add and easy to leave behind once the value beneath it becomes
    ...
    for d in docs:
        ...
        for n, line in enumerate(d.read_text().splitlines(), 1):
            ...
            toks = document_tokens(strip_html_comments(line))
...
def check(rows: list[dict], docs: list[Path]) -> int:
    r"""Fail if a document states a value no artifact supports.

    Deliberately crude: it looks for decimal numbers and asks whether each
    appears in SOME artifact. `BARE` is `\b0\.[0-9]{3,17}\b`, so the band
    it covers starts at three decimals -- which is also the band where token
    membership is least selective. That cannot catch a right number attached to the
    ...
    def _lineage_status(src: str) -> str:
        ...
        UNLISTED IS A FAILURE, not a default.

        "Unlisted defaults to canonical" stops the registry hiding a result by
        omission, and opens the other edge: 28 unlisted lineages, 58 of whose
        manifests do not meet the full-SHA-plus-tree standard, and a NEW
        unlisted lineage could back a
    ...
    def _provenanced(src: str) -> bool:
        ...
        status = j.get("status")
        if sha is None:
            # A MISSING SIBLING IS ABSENCE, NOT CORRUPTION. This probe is
            # speculative: it runs only for artifacts carrying no
            # `code_git_sha` of their own, and some of those do not sit beside
            # a manifest at all. `derived/` is the case in hand -- it holds
            # `sbom.cdx.json`, a CycloneDX document with no provenance block
            # and no `derived/manifest.json` to fall back to.
            #
            # Routing the probe through `_load` filed that absence in MALFORMED
            # and failed the gate with `derived/manifest.json:
            # FileNotFoundError`, naming a file nothing ever claimed existed.
            # Worse, `srcs` at the call sites is a SET and the caller
            # short-circuits on `any()`, so whether the probe ran at all
            # depended on set iteration order: the gate's EXIT CODE varied with
            # PYTHONHASHSEED on byte-identical inputs. Seeds 0 and 10 failed
            # while 1-9 and 11 passed, at this commit and at 09e7305 before it,
            # so every green run of this check was green by luck. The companion
            # fix is `sorted(srcs)` at those call sites.
            #
            # A sibling that EXISTS and is corrupt is still a defect and is
            # still recorded. Only the not-there case is quiet now.
            mani = f.parent / "manifest.json"
            sib = _load(mani) if mani.is_file() else None
            sib = sib if isinstance(sib, dict) else {}
            sha, status = sib.get("code_git_sha"), sib.get("status", status)
        # A FULL SHA. Accepting any non-"unknown" string lets a
        # twelve-character abbreviation count as provenance, and an
        # abbreviation cannot be resolved in a clone that does not already
        # hold the object -- which is the situation of every reader.
    ...
    for d in docs:
        ...
        for n, line in enumerate(_lines, 1):
            ...
            # them.
            # STRIP THE MARKER FIRST. It contains the word "historical",
            # which is itself a cue, so testing the raw line made this a
            # tautology and the diagnostic below dead code.
            # AND LOOK AT THE PARAGRAPH, NOT THE LINE. A retraction is
            # narrated across several lines, while the marker must sit on
            # the line carrying the VALUE -- so the narrating verb is
            # routinely two lines above it. Checking the marker's own line
            # alone flagged fifteen genuine narrations as bypasses.
            # TWO LINES, and the prose must carry the verb. A retraction is
            # narrated in a paragraph while the marker must sit on the line
            # carrying the VALUE, so the narrating verb is routinely a few
            # lines above -- but a fixed nine-line reach crossed into
            # unrelated prose and 50.7% of windows then contained some cue by
            # chance, which is a coin flip, not a guard. Walking back to the
            # chance, which is a coin flip, not a guard. Two lines holds the
            # base rate at 12.7%, and where a genuine narration falls outside
            # it the DOCUMENT is fixed, not the window.
            without_marker = " ".join(
                _lines[max(0, n - 2):n]).replace(HISTORICAL, " ").lower()
            narrates = (HISTORICAL in line
                        and any(c in without_marker for c in RETRACTION_CUES))
            if not narrates:
            ...
            prose_only = strip_html_comments(line)
            ...
            for tok, cands in pairs:
                ...
                # SORTED, BECAUSE `any`/`all` SHORT-CIRCUIT. `srcs` is a set,
                # so unsorted iteration made WHICH sources get inspected -- and
                # therefore which side effects `_provenanced` records -- depend
                # on PYTHONHASHSEED. The diagnostics below already sorted; the
                # decisions did not. A gate whose verdict moves with the hash
                # seed verifies nothing.
                elif not any(_provenanced(x) for x in sorted(srcs)):
                    ...
                    unprovenanced += 1
                elif all(_superseded(x) for x in sorted(srcs)
                         if _provenanced(x)):
                    ...
                    stale_lineage += 1
                             ...
                             for x in sorted(srcs) if _provenanced(x)):
    ...
    globals()["LAST_COUNTS"] = {
        ...
        # NAMED EXEMPTIONS WERE PRINTED AND NEVER EXPORTED, so the count of
        # them that README quotes was gated by nothing while the count beside
        # it was gated. Half a checked sentence.
        "named_exempt": named_exempt,
    ...
    print("  NOT checked here: that a value measures what its sentence says "
          "it measures (no tool checks this; check_units.py compares units BETWEEN ARTIFACTS and never reads a document), that a named "
