...
def permutation_spread_null(patterns_dir: Path, bundle: Path, budget: int,
    ...
    # `metrics.py (ring_coverage_concentration)`, where the same quantity is re-exported as
    ...
    return {
        "estimand": (
               ...
               "rings, and metrics.py (ring_coverage_concentration) renames the metric to stop that "
               "reading."),
        "budget": budget,
        "draws": n_draws,
        ...
        # (src/aml/eval/metrics.py (MEMBERSHIP IS MANY-TO-MANY)).
        #
...
def _interpretation(per: dict, spans_one: int, n_sig: int, holm_ci: list,
    ...
    fixed = (
        ...
        "harder to detect (see metrics.py (ring_coverage_concentration)). ")
    order_note = (
