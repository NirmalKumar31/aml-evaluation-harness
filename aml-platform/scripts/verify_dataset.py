...
def main(argv=None) -> int:
    ...
    found = scan(a.data)
    if not found:
        # ⛔ THIS RETURNED 2 ON AN EMPTY DATA DIRECTORY, which made
        # `make verify-data` -- a step the README lists in its quick start --
        # the first hard failure a stranger hits on the documented path, in a
        # clone that is SUPPOSED to have no dataset. It also contradicted this
        # script's own contract: `--require` is what a release gate uses, and
        # absence without it is a fact to report, not an error. Exit 0 and say
        # so; `--require` still fails closed below.
        print(f"no dataset files under {a.data}/ -- nothing to verify. "
              f"AMLworld is not redistributed; see `make get-data`. "
              f"(Pass --require to make absence a failure.)")
        if a.require:
            print("--require was given, so this is a failure", file=sys.stderr)
            return 2
        return 0
