...
def main(argv=None) -> int:
    ...
    rows = con.execute(f"""
        ...
        FROM read_csv_auto('{a.src}')
        GROUP BY 1
        -- ⛔ ORDER BY 3 DESC ALONE IS NOT A TOTAL ORDER. Two formats have
        -- ZERO positives (Wire and Reinvestment), so the sort key ties and
        -- DuckDB's parallel group-by resolves it by scan order: two runs of
        -- this script on the same CSV emit `by_payment_format` in different
        -- orders, and `formats_with_zero_positives` flips with it. An audit
        -- caught it by regenerating the artifact and diffing. This is the same
        -- species as the `ORDER BY txn_id` defect the project retracted a
        -- result over -- an unordered scan feeding a published artifact.
