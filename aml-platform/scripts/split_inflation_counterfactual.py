"""Decompose the split-inflation AP gap without assuming AP scales LINEARLY
with prevalence.

⚠️ IT IS NOT ASSUMPTION-FREE, and this docstring used to say it needed "no
model of how AP responds to prevalence". Resampling the excluded positives'
scores i.i.d. from the retained positives is algebraically identical to
up-weighting the retained positives by lambda = n_all / n_retained: an audit
reproduced the published counterfactual AP to within 0.3 Monte-Carlo standard
errors with a one-line weighted AP and no simulation. So the estimand carries
an EXCHANGEABILITY assumption -- that the excluded positives would have scored
like the retained ones -- and the 2,000 AP evaluations are a way of computing a
functional form, not of avoiding one. That assumption is the whole content of
the decomposition, not a footnote, and "53% detectability" is one GBDT's score
gap rather than a model-independent quantity.
...
def main(argv=None) -> int:
    ...
    # ONE GENERATOR PER SEED, not one threaded through all of them. A single
    # rng advanced across five `decompose` calls makes row k depend on how many
    # rows preceded it, so `--seeds 2` does not reproduce the seed-2 row of a
    # five-seed run. Same species as the row-order defect this project
    # retracted a result over.
    per_seed = {}
    for seed in (int(x) for x in a.seeds.split(",")):
        t0 = time.time()
        d = decompose(load_seed(a.gold, a.variant, seed), a.draws,
