...
A common position is that the difference is unmeasurable — that because the excluded rows are gone, the amount by which a naive protocol flatters a result cannot be quantified or corrected for. This experiment is the test of that position.
