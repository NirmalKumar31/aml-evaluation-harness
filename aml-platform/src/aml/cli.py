...
def build_parser() -> argparse.ArgumentParser:
    ...
    # allow_abbrev=False, because argparse's prefix matching silently changed a
    # published number. `model-stability` and `leak-sweep` expose `--seeds`
    # (default "0,1,2,3,4") while every neighbouring subcommand exposes
    # `--seed`. So `--seed 3` was accepted as `--seeds=3` and collapsed a
    # FIVE-SEED stability run to one seed, with no warning -- in the project
    # whose central finding is that a single-seed `precision@50` is not a
    # result. The same typo collapsed `leak-sweep`'s placebo to one draw.
    p = argparse.ArgumentParser(prog="aml", allow_abbrev=False)
    # ⛔ `allow_abbrev=False` ON THE TOP-LEVEL PARSER IS A NO-OP, and the first
    # version of this fix was exactly that. argparse does NOT propagate the
    # flag: `_SubParsersAction.add_parser` constructs each subparser with the
    # default `True`, so `model-stability --seed 3` was still silently
    # accepted as `--seeds=3` while an eight-line comment above asserted the
    # defect was fixed. The top-level parser owns only `-h` and `cmd`, so the
    # flag could never have changed anything there.
    #
    # Every subparser is built through this wrapper instead, and
    # `test_seed_is_not_silently_accepted_as_seeds` fails if it regresses.
    sub = p.add_subparsers(dest="cmd", required=True)
    _orig_add_parser = sub.add_parser

    def _no_abbrev_add_parser(name, **kw):
        kw.setdefault("allow_abbrev", False)
        return _orig_add_parser(name, **kw)

    sub.add_parser = _no_abbrev_add_parser
