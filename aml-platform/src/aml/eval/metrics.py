...
def recall_at_budget(ad: pd.DataFrame, budget: int) -> dict:
    ...
    #
    # rank(method="first") resolves ties by input row order. Row order here
    ...
    #
    # ⛔ TWO THINGS THIS COMMENT USED TO SAY ARE WRONG.
    #
    # It blamed "gradient-boosted ensembles emit repeated leaf scores in
    # quantity". The LOGISTIC baseline has the same tie signature -- about 1.73
    # rows per distinct (day, score) in the top 1000, against 1.72 for the
    # GBDT. A logistic fit on continuous features does not collide in float64.
    # The ties are made HERE, by `to_account_days`: one transaction emits two
    # account-days carrying the same score. It is a property of the alert unit,
    # not of the model.
    #
    # And the bracket is therefore far wider than the reachable set. Every
    # boundary tie group measured on the archived bundles has size 2 and
    # `all_same_label` -- it is the sender/receiver pair of ONE transaction,
    # and `y = max` gives both endpoints the same label. `min` and `max`
    # promote or demote both together, which no budget can do, so the two ends
    # of the bracket are not attainable by any tie-break. Re-ranking the
    # bundles under 200 random within-day row orders moves `recall@k` by ZERO
    # on HI-Medium and HI-Small and by at most one positive in 61,698 on
    # HI-Large, against published brackets 14 to 60 wide. Read the bracket as
    # an upper bound on an exposure that is, on this data, empty.
    #
...
def bootstrap_ci(df: pd.DataFrame, score: np.ndarray, budget: int = 50,
    ...
    CLUSTERS
        ...
        cannot be ring-clustered, so each unringed positive ACCOUNT-DAY becomes
        its own cluster -- `_dependence_clusters` returns one label per
        positive account-day, not per account. This docstring said "each
        account", which is a different and smaller number: 104 unringed
        positive account-days over 92 accounts on the frame measured, so an
        account laundering on nineteen days counts as nineteen independent
        observations.
