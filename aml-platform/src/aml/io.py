...
# ...and anything that LOOKS like a scheme but does not match `_URI` is an
# error, never a local path. `is_uri("abfss:/c@a.dfs.core.windows.net/x")` --
# one missing slash -- returned False, so the caller treated it as relative
# and `ensure_dir` cheerfully created a local directory tree named
# `abfss:/c@a.dfs.core.windows.net/x`. That is verbatim the failure this
# module's docstring exists to prevent: a remote write silently becoming a
# local one. A leading space did the same. Fail loudly instead.
# A SLASH AFTER THE COLON, and a scheme of 2+ characters. Without both, this
# refused legitimate input: `C:tmp` is a Windows drive-relative path, and
# `data:raw/x` / `runs:2026/x` are ordinary relative paths whose first segment
# contains a colon. Requiring `:/` keeps the real target -- a URI missing one
# slash -- while letting those through.
_SCHEME_LIKE = re.compile(r"^\s*[A-Za-z][A-Za-z0-9+.\-]+:/")

# ...and the remote schemes this project actually uses, caught even when BOTH
# slashes are missing. Requiring `:/` alone let `abfss:c@a/x` through --
# strictly worse than the one-slash typo the guard was written for, because
# it is further from a valid URI and still resolved to a local path.
_REMOTE_SCHEME = re.compile(
    r"^\s*(?:abfss|abfs|adl|az|s3|s3a|gs|gcs|wasb|wasbs|https?|ftp|file|memory):",
    re.IGNORECASE)


class MalformedURI(ValueError):
    """A path looks like a URI but is not one. Never silently treated as local."""


def _reject_malformed_uri(path: str) -> None:
    text = str(path)
    if _URI.match(text):
        return
    scheme_like = _SCHEME_LIKE.match(text) or _REMOTE_SCHEME.match(text)
    if scheme_like and not re.match(r"^[A-Za-z]:[\\/]", text):
        raise MalformedURI(
            f"{text!r} looks like a URI scheme but is not a well-formed URI. "
            f"Refusing to treat it as a local path -- a missing slash or a "
            f"leading space would otherwise create a local directory named "
            f"after the remote target. Expected `<scheme>://...`.")
...
def is_uri(path) -> bool:
    ...
    ⛔ THE MALFORMED-URI GUARD LIVES HERE, not at one call site. It was first
    placed in `ensure_dir` only, and `grep -rn _reject_malformed_uri src/`
    found exactly that one caller -- so `write_json("abfss:c@a/x.json", ...)`
    and `write_text("s3:bucket_key.txt", ...)` both returned True and created
    LOCAL files named after the remote target, which is the failure the guard
    exists to prevent. `is_uri` is the chokepoint every reader and writer in
    this module consults to decide local-versus-remote, so checking here
    covers all of them by construction rather than by enumeration.
    """
    _reject_malformed_uri(path)
...
def ensure_dir(path) -> None:
    ...
    And a path that LOOKS like a URI but is malformed is rejected rather than
    created locally -- see `_reject_malformed_uri`.
    """
    _reject_malformed_uri(path)
    if is_uri(path):
        fs, p = _fs(path)
