...
# A cap high enough that no real traceback reaches it, and a flag when it does.
_CHAIN_CAP = 512


def _chain(exc: BaseException, limit: int = _CHAIN_CAP):
    ...
    ⛔ THE LIMIT USED TO BE 20, AND `classify` CALLED IT WITH THE DEFAULT.
    `classify`'s own docstring says "correctness is checked across the whole
    chain", and it was not: a `LeakageError` at depth 25 under transient
    wrappers was never inspected, so `classify` fell through to the transient
    branch and the orchestrator RETRIED A LEAKAGE ASSERTION -- the one outcome
    this module exists to prevent, on a stage that costs six hours. Cycles are
    already prevented by `seen`, so the cap is now only a runaway guard, and
    hitting it is reported rather than silently truncating.
    """
...
def classify(exc: BaseException) -> int:
    ...
    # FAIL CLOSED ON TRUNCATION. If the ancestry is deep enough to hit the
    # runaway cap we have not inspected all of it, and "not retryable" is the
    # only safe answer -- retrying past an unseen correctness stop is the
    # expensive mistake, not stopping on a transient one.
