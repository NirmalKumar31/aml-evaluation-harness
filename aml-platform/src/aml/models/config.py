...
#
# ⛔ AND THE CORRECTION TO THAT CORRECTION WAS ALSO WRONG.
#
# This comment first named `balanced` as the likeliest cause of the 37%
# run-to-run spread. It was then rewritten to say `balanced` "CANNOT be" the
# cause, because it is a deterministic function of `y` -- the same vector
# every run, and a quantity that never varies cannot be a source of variation.
# That reasoning is wrong at the pinned sklearn, and the wrongness is one line
# of sklearn source:
#
...
#
# The weights are not an input the draw ignores -- they ARE the probability
# vector of the draw, and `BaseHistGradientBoosting.fit` passes them in via
# `_bin_data(X, sample_weight, is_training_data)`. So `balanced` does not vary
# between runs and still amplifies how much the SEED varies: a deterministic
# quantity can reshape a random draw.
#
# ⚠️ AND THE FIRST VERSION OF THIS PARAGRAPH REPORTED ONE DATA DRAW AS IF IT
# WERE THE MAGNITUDE -- "roughly a fourfold amplification, and up to
# eightfold". That was `default_rng(0)`, which is the most favourable of eight.
# Measured at 210,001 rows and 0.08% prevalence, max |delta bin threshold|
# across 6 seeds, over 8 independent data draws:
#
#     ratio balanced/unweighted:  4.20 2.18 4.08 1.52 2.37 1.59 2.00 3.45
#     median 2.28, range 1.52-4.20
#
# So the amplification is real and present in every draw, and its central
# value is about 2.3x rather than 4x. Writing a single draw as the headline is
# the error this project retracted a spread over; it was committed here while
# correcting a different error in the same comment. At
# this prevalence the balanced draw puts ~50% of its mass on ~15,700 distinct
# positives, so the 200,000-row subsample is a heavy duplication bootstrap of
# a few thousand rows rather than a quantile estimate over the corpus.
#
# The source of the variation is still the bin mapper. But `balanced` is not
# innocent of it, and the previous two versions of this comment were wrong in
# opposite directions.
#
