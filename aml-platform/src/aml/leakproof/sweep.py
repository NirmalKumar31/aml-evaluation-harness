...
⚠️ TWO DIFFERENT QUANTITIES IN THIS FILE ARE CALLED "noise floor", and
conflating them is how a misnomer reached six documents:

  (a) the 15.5% above -- identical data, hyperparameters and seed, ONLY the
      training row order permuted. A within-configuration dispersion of one
      quantity. This one is a floor: a ratio inside it is not distinguishable
      from 1.00 on a single fit, which is the argument this module rests on.

  (b) `summarise()`'s `noise_floor.placebo_spread_pct` -- 46.10% -- which
      pools the placebo ratios of three DIFFERENT channels. It is not a
      floor and nothing is compared to it. See the block comment there.

Only (a) licenses the reasoning above. (b) must not be quoted as a floor, and
no threshold may be derived from either -- (a) is HI-Small, one cut, one model
family, four fits.
...
def summarise(per_seed: list, kinds) -> dict:
    ...
    # ⛔ THE KEY BELOW IS CALLED `noise_floor` AND IT IS NOT ONE.
    #
    # `placebo_spread_pct` pools the placebo AP ratios of ALL THREE channels
    # and takes (max - min) / mean. The endpoints belong to DIFFERENT
    # channels -- the three channel placebo means are 0.8834, 0.9841 and
    # 0.7713 -- so the 46.10% it reports mixes a between-channel LEVEL SHIFT
    # with within-channel seed noise. It is a pooled dispersion, not a floor
    # that anything is measured against: every verdict in `ladder` is decided
    # by `real.min() > placebo.max()` against that channel's OWN placebo, and
    # nothing in this function or downstream of it ever compares a value to
    # `placebo_spread_pct`.
    #
    # The quantity that WOULD be a noise floor is the within-channel spread,
    # and it cannot be recovered from the canonical archive: `leaksweep2_Small/`
    # ships no `sweep_raw.json`, so the per-fit draws are gone.
    #
    # The name is retained because it is the key in the archived
    # `sweep_summary.json` and that run is not regenerable here; renaming it
    # would leave the generator and its artifact permanently inconsistent.
    # It must NOT be read as licensing "noise floor" language anywhere, and
    # must never be used to derive a detection threshold. The full retraction
    # is `paper/RESULTS_leak_detection.md` §1b item 2. Six documents had
    # already inherited the misnomer from this dict by the time it was caught.
    plac_all = np.concatenate([col(f"{k}::placebo", "average_precision_ratio")
                               for k in kinds])
    return {
        "ladder": rows,
        # MISNOMER, see above: a pooled between-channel dispersion.
        "noise_floor": {
