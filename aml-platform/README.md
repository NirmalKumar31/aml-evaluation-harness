...
```bash
aml demo --dest /tmp/demo   # generates a corpus, runs every stage, ~2 s warm
```
...
**No derived row-level records are distributed.** Nine replay bundles —
**526,355 rows** of calendar day, opaque account code, label, model score,
rank and ring linkage — exist so that a published budget metric can be
recomputed without the dataset, but they are held privately and are in no
distribution: not the repository, not the image, not the wheel or sdist.
Whether redistributing them would be permitted under CDLA-Sharing-1.0 is
**unreviewed**, which is why they are withheld; each bundle records
`licence_status: UNREVIEWED`. `results_archive/replay_inventory.json` is the
aggregate inventory that does ship. `DATA_LICENSE.md` — in the wheel's
`dist-info/licenses/`, at the root of the sdist, and beside this file in the
repository — is the full analysis.
