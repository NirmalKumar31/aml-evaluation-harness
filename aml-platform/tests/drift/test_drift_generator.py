...
Five invariants the drift generator must hold:
...
def _load(dest):
    ...
    return (q("drift_rings"), q("drift_ring_txns"), q("drift_ring_accounts"),
            # `Path.read_text` on both branches. The `else` used a bare
            # `open(...).read()`, which leaks the handle until GC.
            json.loads((dest / "drift_manifest.json").read_text())
            if hasattr(dest, "read_text") else
            json.loads(Path(f"{dest}/drift_manifest.json").read_text()))
