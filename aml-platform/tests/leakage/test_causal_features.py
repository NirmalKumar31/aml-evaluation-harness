...
def test_same_minute_repeats_inflate_the_degree_and_that_is_the_documented_behaviour(tmp_path):
    """The name says distinct; the arithmetic counts first-sightings.

    Every history frame here stops at `INTERVAL 1 MINUTE PRECEDING` to stay
    causal, so `is_new_cp` asks "had this account dealt with this counterparty
    as of a minute ago?". Two transactions between the SAME PAIR in the SAME
    MINUTE both answer no, both are flagged new, and the running total counts
    one counterparty twice.

    The test above uses one transaction per DAY and therefore never reaches
    this case; the online path mirrors the same rule, so the parity test agrees
    on the wrong number too. That is how a feature named
    `n_distinct_cp_ever` shipped without being a distinct count -- and it is
    the feature set behind the graph-ablation finding.

    This pins the behaviour deliberately. The flag is correct for an online
    scorer a minute behind, so the fix was to the CLAIM rather than the
    arithmetic; if the semantics are ever changed, this test should fail and
    the change should be a decision, not a silent one.
    """
    rows = [{**BASE, "event_time": "2022-09-01 10:00", "amount_paid": 100.0,
             "receiver_account": "B"},
            {**BASE, "event_time": "2022-09-01 10:00", "amount_paid": 101.0,
             "receiver_account": "B"},
            {**BASE, "event_time": "2022-09-01 10:05", "amount_paid": 102.0,
             "receiver_account": "B"}]
    f = _feats(tmp_path, rows, "gsameminute")

    # A has ever dealt with exactly ONE counterparty. The third row sees 2.
    assert f.s_n_distinct_cp_ever.tolist() == [0, 0, 2], (
        "same-minute repeats no longer inflate n_distinct_cp_ever -- if that "
        "was deliberate, update docs/LIMITATIONS.md on finding #4, which was "
        "measured on the old behaviour, and this test")
